            ...
            {game.name? (
                <div className='container flex'>
                    <div className='flex flex-col w-3/4'>
                        <img className='h-3/4 w-3/4 img' src={game.background_image ? game.background_image : game.image} alt={game.name}/>
                        
                        <hr className='w-4/6 m-4'/>
                        <p className='text-white text-sm w-5/6' dangerouslySetInnerHTML={{__html: game.description}}/>
                    </div>
                    <div className='container text-white w-2/4'>
                        <h1 className='text-7xl text-center'>{game.name}</h1>
                        <div className='flex place-content-evenly m-4'>
                            {game.publishers ? <h3>{game.publishers[0].name}</h3> : <></>}
                            {game.released ? <h3>{game.released}</h3> : <h3>{game.release_date.split('-')[2] + ' - ' + game.release_date.split('-')[1] + ' - ' + game.release_date.split('-')[0]}</h3>}
                            <h3>{`${game.rating}⭐`}</h3>
                            {game.website ? <div className='cube w-16 h-16'>
                                <div className='cubeContent text-white'>
                                    <a href={game.website}>{web}</a>
                                </div>
                            </div> : <></>}
                        </div>
                        <hr className=''/>
                        <h2 className='m-4 text-4xl text-center'>Genres</h2>
                        
                        <div className='flex place-content-evenly m-4 text-center'>
                            {
                                game.genres.map(e => {
                                    return(
                        ...
                        {game.stores ? <hr className=''/> : <></>}
                        {game.stores ? <h2 className='m-4'>Stores</h2> : <></>}
                        <div className='flex flew-row place-content-evenly m-4'>
                            {
                                game.stores ? 
                                game.stores.map(e => {
                                    let aux = [];
                                        if(e.store.name.toLowerCase().includes('playstation')) {aux.content = playstationStore}
                                        else if(e.store.name.toLowerCase().includes('360')) {aux.content = xbox360Store}
                                        else if(e.store.name.toLowerCase().includes('xbox')) {aux.content = xboxStore}
                                        else if(e.store.name.toLowerCase().includes('steam')) {aux.content = steamStore}
                                        else if(e.store.name.toLowerCase().includes('epic')) {aux.content = epicgamesStore}
                                        else if(e.store.name.toLowerCase().includes('gog')) {aux.content = gogStore}
                                        else if(e.store.name.toLowerCase().includes('nintendo')) {aux.content = nintendoStore}
                                        else if(e.store.name.toLowerCase().includes('app')) {aux.content = appleStore}
                                        else if(e.store.name.toLowerCase().includes('play')) {aux.content = playStore}
                                        else if(e.store.name.toLowerCase().includes('itch')) {aux.content = itchioStore}
                                        else {aux.content = web}
                                        return (
                                            <div key={e.store.id} className='cube'>
                                                <div className='cubeContent w-16 h-16'>
                                                    <a href={`https://${e.store.domain}`}><img src={aux.content} alt='logo'/></a>
                                                </div>
                                            </div>
                                        )
                                    })
                        ...
                        <hr className=''/>
                        <h2 className='m-4'>Platforms</h2>
                        <div className='flex flew-row place-content-evenly m-4'>
                            {
                                game.parent_platforms ? 
                                game.parent_platforms.map(e => {
                                    let aux = {}
                                        if(e.platform.name.toLowerCase().includes('playstation')) {aux.content = playstation}
                                        else if(e.platform.name.toLowerCase().includes('xbox')) {aux.content = xbox}
                                        else if(e.platform.name.toLowerCase().includes('pc') || e.platform.name.toLowerCase().includes('linux') || e.platform.name.toLowerCase().includes('mac')) {aux.content = pc}
                                        else if(e.platform.name.toLowerCase().includes('ios') || e.platform.name.toLowerCase().includes('android')) {aux.content = mobile}
                                        else if(e.platform.name.toLowerCase().includes('nintendo')) {aux.content = nintendo}
                                        else if(e.platform.name.toLowerCase().includes('web')) {aux.content = internet}
                                        else {aux.content = others}
                                        return (
                                            <div key={e.platform.id} className='cube'>
                                                <div className='cubeContent w-16 h-16'>
                                                    {aux.content}
                                                </div>
                                            </div>
                                    ...
                                    if(e.toLowerCase().includes('playstation')) {aux.content = playstation}
                                    else if(e.toLowerCase().includes('xbox')) {aux.content = xbox}
                                    else if(e.toLowerCase().includes('pc') || e.platforms.name.toLowerCase().includes('linux') || e.platform.name.toLowerCase().includes('mac')) {aux.content = pc}
                                    else if(e.toLowerCase().includes('ios') || e.platforms.name.toLowerCase().includes('android')) {aux.content = mobile}
                                    else if(e.toLowerCase().includes('nintendo')) {aux.content = nintendo}
                                    else if(e.toLowerCase().includes('web')) {aux.content = internet}
                                    else {aux.content = others}
                                    return (
                                        <div key={e + 'db'} className=''>   
                                            <div className=''>
                                                {aux.content}
                                            </div>
                                        </div>
